  ...
  public:
    ...
    private:
		...
		double avg_rtt[MAXF];      //average measured rtt;
                double tot_weit[MAXF];     //record the additive increase window size 
		double GRATE[MAXF];         //total guaranteed rate
		double TOT_MRATE;           //total guaranteed minimum rate
...
leaf<DType>::leaf()
{ 
    traf_rate=prog_delay=0; id=pkt_sz=tor_num=0; TOT_MRATE=0;
...
leaf<DType>::~leaf()
{
  traf_rate=prog_delay=0; id=pkt_sz=tor_num=0; num_send_flow=num_recv_flow=TOT_MRATE=0;
  ...
  { bit_num[i]=bit_sent[i]=bit_received[i]=bit_acked[i]=bit_to_rec[MAXF]=0; 
    ...
    send_flow[i]=recv_flow[i]=send_flow_id[i]=recv_flow_id[i]=GRATE[i]=0; 
	MP[i].kill(); last_pkt_sz[i]=0; 
  }
...
void leaf<DType>::SetUp()
{
  ...
  { bit_num[i]=bit_sent[i]=bit_received[i]=bit_acked[i]=bit_to_rec[i]=0; 
    alpha[i]=0; retran_ID[i]=0; retran_time[i]=0; avg_rtt[i]=GRATE[i]=0;
    tcp_wd_sz[i]=tcp_ack_num[i]=rep_ack[i]=tcp_thd[i]=tcp_cong[i]=ecn_count[i]=0; ecn_flag[i]=false; 
 ...
 for (int i=0; i<MAXF; i++) {ASF.EnQueue(i); ARF.EnQueue(i);}
  num_send_flow=num_recv_flow=0; TOT_MRATE=0;
...
{ pkt p;
  ...
  if(hostclass==2) { //set up deadline of qos flow
	  ...
	  mrate=100.*nbits/(deadtime[i]-tm);
	  if(mrate+TOT_MRATE<TMRG) { GRATE[i]=mrate; TOT_MRATE+=mrate;}  //accept as MRG flow
	  else { deadtime[i]=INF; GRATE[i]=0;}                      //treat as BE flow
  }
...
{ 
  ...
  if(n==0) { //end a receiving flow
    ...
    MP[f].kill(); num_recv_flow--; recv_flow[f]=-1; recv_flow_id[f]=-1; ARF.EnQueue(f);
	 ...
	 TOT_MRATE-=GRATE[f]; GRATE[f]=0; if(num_send_flow==0) { TOT_MRATE=0;}
         tot_weit[f]=0; send_flow[f]=-1; send_flow_id[f]=-1; ASF.EnQueue(f); //tot_lfn--;
   }
...
{
   int n,nn,sr,ds,ff,acknumb,mrate;
   int i,j,k,key,index,ii;
   double weit, frac, dvalue, pvalue, gvalue, tc, tm,tt;
   bool ackflag;
   pkt pp, pk, pck;
   List<pkt>  cp;
   ...
   if(hostclass==0) { weit=1; mrate=0; }
     ...
     if(GRATE[ff]>0) {mrate=(int) (((bit_num[ff]-nn-1)*pkt_sz+last_pkt_sz[ff])*avg_rtt[ff]/pkt_sz/(deadtime[ff]-tm));}
	 else { mrate=0; }
   }
   weit=1.0*weit/num_send_flow;
        ...
        if(nn<bit_acked[ff]) { return key; } //discard the repeated ack
		    ...
		    if(tcp_wd_sz[ff]<tcp_thd[ff]) { tcp_wd_sz[ff]=(int)(tcp_wd_sz[ff]*(1-pow(2,-weit))); }
                    else { tcp_wd_sz[ff]=(int)(tcp_wd_sz[ff]/2+(int)(weit-1));}  
	         ...
	         if(tcp_wd_sz[ff]<mrate) { tcp_wd_sz[ff]+=(int)(ii*(rcos-1)+get_random()); 
                       if(tcp_wd_sz[ff]>mrate) { tcp_wd_sz[ff]=mrate;}
                          ...
                          if(tcp_cong[ff]>=tcp_wd_sz[ff]) 
                          { tcp_cong[ff]-=tcp_wd_sz[ff]; tot_weit[ff]+=weit;
                            while(tot_weit[ff]>=1) { tcp_wd_sz[ff]++; tot_weit[ff]-=1; }
                          }
                        }	
	            }
